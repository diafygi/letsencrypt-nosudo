...
If you wish to give the script access to your user private key, it can accept
that as an optional argument.
...
optional arguments:
  ...
  -r PRIVATE_KEY, --private-key PRIVATE_KEY
                        path to your account private key (optional)
  -e EMAIL, --email EMAIL
  ...
  -s S3_BUCKET, --s3-bucket S3_BUCKET
                        the Amazon S3 bucket to upload the challenge file to;
                        this assumes that the bucket serves as the web server
                        for the challenge domain(s)
user@hostname:~$
```
