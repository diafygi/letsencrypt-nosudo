...
def sign_csr(pubkey, csr, email=None, file_based=False):
    ...
    sys.stderr.write("What is the website directory: ")
    webDir = str(raw_input())
...
""".format(
    ...
    "\n".join("openssl dgst -sha256 -sign user.key -out {0} {1}".format(i['sig_name'], 
i['file_name']) for i in ids),
    ...
    for i in ids:
        os.system("openssl dgst -sha256 -sign user.key -out {0} {1}".format(i['sig_name'],
i['file_name']))
...
""".format(n + 4, i['domain'], responses[n]['uri'], responses[n]['data']))
            os.system("echo \"{0}\" > {1}/{2}".format(responses[n]['data'], webDir, responses[n]['uri']))
            stdout = sys.stdout
            sys.stdout = sys.stderr
